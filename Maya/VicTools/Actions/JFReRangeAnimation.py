...
def execute():
    ...
    def setKeyOnPoseCtrls(posePath):
        ...
        for ctrl in ctrls:
            for attr in ("rotate", "translate", "FKIKBlend"):
                try:
                    if "root_ctrl" in ctrl:
                        cmds.setKeyframe( ctrl+"." + attr, itt="step", ott="step" )
                    else:
                        cmds.setKeyframe( ctrl+"." + attr )
                except: pass
            for attr in ("rootMotionXCtrl", "rootMotionYCtrl", "rootMotionZCtrl"):
                try:
                    cmds.setKeyframe( ctrl+"." + attr, itt="step", ott="step" )
                except: pass
    ...
    def loadPoseAndSetKey(role, frame, poseFrame, posePath, nn ):
        ...
        for root_dir, matId in (("X", 12), ("Y", 13), ("Z", 14)):
            isFollow = cmds.getAttr( role + ":Main.rootMotion" + root_dir + "Ctrl")
            if isFollow < 1:
                offset = frameRootCtrl_mat[matId] - rootCtrl_mat[matId]
                cmds.setAttr(rootCtrl + '.translate' + root_dir, offset)
...
def saveInfo():
    ...
    JFAdvSkeletonAllCommand.openAnimationExporter()
